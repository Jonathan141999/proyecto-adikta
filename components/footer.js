class CustomFooter extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Also listen for the lang ready event
    window.addEventListener('lang:ready', (ev) => {
      // Re-render footer content on language change to update links
      // We do NOT call setLanguage to avoid recursion loops
      this.innerHTML = `
      <footer class="site-footer">
        <div class="footer-inner">
          <div class="footer-col footer-brand">
          ...
          <div class="footer-col footer-support">
            <h5 data-i18n="footer_support_title">SOPORTE</h5>
            <ul>
              <li><a href="${this.getPagePath('legal.html')}" data-i18n="footer_warranty_link">Garantía</a></li>
              <li><a href="${this.getPagePath('legal.html')}" data-i18n="footer_terms_link">Términos y condiciones</a></li>
              <li><a href="${this.getPagePath('legal.html')}" data-i18n="footer_privacy_link">Política de Privacidad</a></li>
            </ul>
          </div>
        </div>
      ...
      `;
      // Re-apply translations specifically for the new footer content if needed, 
      // although lang.js observer might handle it.
      if (typeof window.applyTranslationsDebug === 'function') {
        window.applyTranslationsDebug(window.currentLang || document.documentElement.lang);
      }
    });
  }
}
